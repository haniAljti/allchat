...
class InfoXmppDataSource(
    ...
    override suspend fun getUpdatedEntityInfo(id: String): CallResult<RemoteEntityInfo> {
        val avatarResult = fetchAvatarData(id, null)
        val nicknameResult = fetchNickname(id)
    ...
    override suspend fun fetchAvatarData(id: String, hash: String?): CallResult<Avatar?> =
    ...
    override suspend fun fetchNickname(id: String): CallResult<String?> =
        withContext(Dispatchers.IO) {
            return@withContext try {
                if (pepManager.isSupported) {
                    val pubSubManager =
    ...
    override suspend fun updatedInfoFlow(): Flow<InfoUpdate> {
